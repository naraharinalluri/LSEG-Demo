...
export class WealthDriver {
  private state: WealthState;
  private listeners = new Set<(s: WealthState) => void>();
  private interval: ReturnType<typeof setInterval> | null = null;
  private rng = new SeededRng(101);
  private startMs = 0;
  private firedActivityIds = new Set<string>();
  private firedNewsIds = new Set<string>();
  ...
  private computeClientState(client: Client, instruments: Record<string, LiveInstrument>): ClientState {
  ...
  subscribe(listener: (s: WealthState) => void) {
